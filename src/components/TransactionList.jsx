        ...
        {transaction.map((item) => (
          <div key={item.id} className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-gray-50 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-gray-500">{item.category} · {new Date(item.date).toLocaleDateString("en-GB")}</p>
            </div>

            <div className="flex items-center justify-between gap-4 sm:justify-end">
              <p className={`shrink-0 font-semibold ${item.type === "income" ? "text-green-600" : "text-red-600"}`}> {item.type === "income" ? `+ ${item.amount}` : `- ${item.amount}`} </p>
